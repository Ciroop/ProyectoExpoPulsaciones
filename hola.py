...
try:
  ...
  ser = serial.Serial('/dev/ttyACM0',baudrate = 115200)       # Setting Serial port number and baudrate
  ...
  while 1:                                            # While loop to continuesly scan and read data from serial port and execute
      ...
      if data[0] == "DATAL":                          # Checking if the identifier is "DATAL" which the Arduino sends the data as the gyro X, Y and Z values
        ...
        mouse.move(x + int(data[1]),y + int(data[2]))        # Moving the mouse by using the X and Y values after converting them into integer
